...
class particle:
    ...
    def move_odom(self, odom, noise):
        ...
        rot1_hat = delta_rot1 + np.random.normal(
            0, alpha1 * abs(delta_rot1) + alpha2 * dist
        )
        trans_hat = dist + np.random.normal(
            0, alpha3 * dist + alpha4 * (abs(delta_rot1) + abs(delta_rot2))
        )
        rot2_hat = delta_rot2 + np.random.normal(
            0, alpha1 * abs(delta_rot2) + alpha2 * trans_hat
        )
...
class RobotFunctions:
    ...
    def initialize_particles_gaussian(
        ...
        for p in self.particles:
            # Add Gaussian noise around the clicked position
            p.x = np.random.normal(mean_x, sigma_pos)
            p.y = np.random.normal(mean_y, sigma_pos)
            p.orientation = np.random.normal(mean_theta, sigma_theta)

            # Reset weights to uniform distribution
            p.weight = 1.0 / self.num_particles

        # Reset the weights array in the manager
        self.weights = np.ones(self.num_particles) / self.num_particles
    ...
    def scan_reference(
        ...
        eps = 1e-3
        valid = (
            np.isfinite(ranges) & (ranges > range_min) & (ranges < (range_max - eps))
        )
        if not np.any(valid):
